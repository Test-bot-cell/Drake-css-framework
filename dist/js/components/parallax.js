...
(function (global, factory) {
    ...
    typeof define === 'function' && define.amd ? define('drakeparallax', ['drake-util'], factory) :
    (global = typeof globalThis !== 'undefined' ? globalThis : global || self, global.DrakeParallax = factory(global.Drake.util));
    ...
    function viewport(options = {}) {
      return createObservable(
        (_target, handler) => ({
          disconnect: drakeUtil.observeViewportResize(
    ...
    function toScrollTargets(elements) {
      return drakeUtil.toNodes(elements).flatMap((node) => {
        var _a;
        const parentElement = drakeUtil.scrollParent(node instanceof Element ? node : void 0, true);
    ...
    var Component = defineComponent()({
      ...
      computed: {
        target: ({ target }, $el) => getOffsetElement(target && drakeUtil.query(target, $el) || $el),
        start({ start }) {
          return drakeUtil.toPx(start, "height", this.target, true);
        },
        end({ end, viewport: viewport2 }) {
          return drakeUtil.toPx(
      ...
      observe: [
        ...
        resize({
          target: ({ $el, target }) => [$el, target, drakeUtil.scrollParent(target, true)].filter(isNode)
        })
      ],
      update: {
        read(data, types) {
          ...
          if (!drakeUtil.isVisible(this.$el)) {
            return false;
          }
          if (!this.matchMedia) {
            return;
          ...
          const prev = percent;
          percent = ease(drakeUtil.scrolledOver(this.target, this.start, this.end), this.easing);
          return {
            percent,
        ...
        write(data) {
          ...
          if (isCssProperties(data.style)) {
            drakeUtil.css(this.$el, data.style);
          }
        },
    ...
    function getOffsetElement(el) {
      return el ? "offsetTop" in el ? el : getOffsetElement(drakeUtil.parent(el)) : document.documentElement;
    }
