...
var Component = defineComponent()({
  ...
  events: {
    ...
    handler(e) {
      if (!e.defaultPrevented) {
        e.preventDefault();
        if (e.current instanceof HTMLElement && !matches$1(e.current, selDisabled)) {
          this.show(e.current);
        }
      }
    }
  ...
  methods: {
    show(index = 0) {
      ...
      if (isElement$1(index)) {
        const { source } = toItem(index);
        index = findIndex$1(items, ({ source: src }) => source === src);
      }
      if (!this.panel) {
      ...
      on$1(this.panel.$el, "hidden", () => this.panel = null);
...
function ensureThumb(toggles, items) {
  for (const [i, toggle] of toggles.entries()) {
    ...
    const parent = parents$1(toggle).reverse().concat(toggle).find(
