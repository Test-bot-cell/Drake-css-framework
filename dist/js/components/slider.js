...
(function (global, factory) {
    ...
    typeof define === 'function' && define.amd ? define('drakeslider', ['drake-util'], factory) :
    (global = typeof globalThis !== 'undefined' ? globalThis : global || self, global.DrakeSlider = factory(global.Drake.util));
    ...
    function getWidth(list, index) {
      return util.sumBy(getSlides(list).slice(0, index), (el) => util.dimensions(el).width);
    }
    ...
    function getElLeft(el, list) {
      ...
      return (util.position(el).left + (util.isRtl ? util.dimensions(el).width - util.dimensions(list).width : 0)) * (util.isRtl ? -1 : 1);
    }
    ...
    function getSlides(list) {
      return util.children(list).filter((slide) => slide instanceof HTMLElement);
    }
    ...
    var Component = defineComponent()({
      ...
      computed: {
        ...
        maxIndex() {
          ...
          if (this.center) {
            return Array.isArray(this.sets) ? (_a = util.last(this.sets)) != null ? _a : this.length - 1 : this.length - 1;
          }
          let lft = 0;
        ...
        slides() {
          return util.children(this.list).filter(
            (slide) => slide instanceof HTMLElement && util.isVisible(slide)
          );
        }
      },
      connected() {
      ...
      observe: [
        resize({
          target: ({ list, $el }) => [$el, ...util.children(list)]
        }),
        intersection({
          handler(entries) {
          ...
          target: ({ list }) => util.children(list),
      ...
      update: {
        write() {
          for (const el of this.navItems) {
            ...
            if (index !== false) {
              el.hidden = !this.maxIndex || index > this.maxIndex || Boolean(this.sets && !util.includes(this.sets, index));
            }
          }
          this.reorder();
          if (!this.parallax) {
