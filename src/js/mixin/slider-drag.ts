...
interface DragPointerEvent extends FrameworkEvent<Element> {
    readonly target: Element;
    button?: number;
}

export default defineMixin<DraggableSliderInstance>()({
    props: {
        draggable: Boolean,
    },
    ...
    created() {
        for (const key of ['start', 'move', 'end'] as const) {
            const fn = this[key];
            this[key] = (e: FrameworkEvent) => {
    ...
    events: [
        {
            ...
            handler(e: DragPointerEvent): void {
                if (
                    !this.draggable ||
                    this.parallax ||
    ...
    methods: {
        start(): void {
            ...
            if (this._transitioner) {
                ...
                this.drag.x +=
                    (this._transitioner.getDistance() ?? 0) * (this.percent ?? 0) * this.dir;

                this._transitioner.cancel();
                this._transitioner.translate(this.percent ?? 0);
...
function getDistance(this: DraggableSliderInstance, prev: number, next: number): number {
    return (
...
function hasSelectableText(el: Element): boolean {
    return (
...
function getAngle(pos1: Point, pos2: Point): number {
